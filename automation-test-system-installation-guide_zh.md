...
6. 請安裝人員使用手機連上 `qa_rpt`，實測可正常上網（例如可開啟任一外部網站）。
...
1. PC 開機後，接上螢幕、鍵盤與滑鼠。
2. 以施工帳號登入 PC（見 0.6 帳號與密碼資訊）。
3. 確認目前 Wi‑Fi 連線為 `qa_rpt`。
4. 於 PC 端開啟瀏覽器確認可正常上網（任一外部網站可開啟）。
...
1. 將螢幕、鍵盤、滑鼠接到機械手臂控制端（RobotArm 電腦）。
2. 開機後登入桌面（帳號密碼見 0.6）。
3. 確認目前 Wi‑Fi 連線為 `qa_rpt`。
4. 開啟瀏覽器確認可正常上網（任一外部網站可開啟）。
5. 若未連上 `qa_rpt`，先完成連線再進行後續安裝/測試步驟。
