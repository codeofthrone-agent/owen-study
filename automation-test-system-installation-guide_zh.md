...
### 2.0.1 交接時「現場電腦操作」最小動作清單（給新手）
> 目的：讓台北團隊可直接接手遠端測試，避免只說「裝好了」但資訊不足。
...
1. 在現場主機建立交接訊息（可貼在群組）：
   ...
   - 網路狀態：`qa_rpt` 已連線（是/否）
   - 阻塞項目：`無 / <Blocked項目>`
2. 上傳 3~5 張現場照片（全景、Wi‑Fi設備、IPCAM、主機與Hub配線、機械手臂區）。
3. 確認主機維持開機、螢幕不鎖定、省電不休眠（至少 2 小時）。
4. 將滑鼠游標停在桌面，關閉與測試無關的視窗，避免干擾遠端操作。
5. 在交接訊息加上「可開始遠端測試」並 @ 台北團隊窗口。
