...
### 1.3 IPCAM 安裝
1. 固定 IPCAM 至預定區位（入口、主通道、手臂作業區優先）。
2. 調整視角避免逆光、鏡面反射與遮擋。
3. 連接電源與資料線（依機型 USB/PoE）並貼上標籤（`CAM-01`、`CAM-02`...）。
4. 併入測試網路（SSID：`qa_rpt`）。
5. 驗證串流可讀且畫面穩定。
...
### 1.4 Speaker / 分音器 安裝
1. Speaker 依分區均勻配置，避免集中單側。
2. 分音器（Audio Splitter）接於主音源與多顆 Speaker 間。
3. 線材固定並避開高干擾或高熱區。
4. Speaker 與分音器分別貼標（`SPK-01`、`AUD-SPLIT-01`）。
5. 逐路播放測試音，確認各分支輸出正常。
...
### 1.5 FP2（Sensor）安裝
1. 將 FP2 固定在建議高度與視角，覆蓋主要感測區域。
2. 避免被大型金屬物、玻璃反射或高熱源直接干擾。
3. 連接電源與網路（依實際架構透過 Hub/閘道）。
4. 完成配對與命名（`SEN-FP2-01`）。
5. 執行區域觸發測試（有人/移動/佔用狀態）。
