...
- **Handoff（交接）**：當 Chapter 2 全部接線與安裝完成後，通知台北團隊進行遠端功能測試與最終確認。

### 2.0.1 交接時「現場電腦操作」精簡版（新手先照這 4 步）
> 原則：主章只保留「一定要做」；細節放附錄，避免新手被資訊淹沒。

1. 送出交接訊息（時間/人員/主機名稱或IP/阻塞項目）。
2. 附 3 張照片（全景、配線、主機畫面）。
3. 主機保持可遠端連線（開機、不鎖定、不休眠）。
4. @ 台北團隊窗口並收到「可開始遠端測試」回覆。
...
> 需要逐項操作細節時，請看 **Appendix E — 交接訊息模板與電腦操作細節（進階）**。
---
...
- [ ] Chapter 2 交接訊息已送出，且台北團隊已回覆可進行遠端測試
...
| IPCAM 無串流 | 網路未連上或供電不穩 | 先重插電源並確認連線到 `qa_rpt` | 記錄 CAM 編號並通知台北團隊協助 |
...
網路狀態：qa_rpt 已連線（是/否）
阻塞項目：無 / <Blocked項目>
附件：現場照片 3 張（全景/配線/主機畫面）
請台北團隊協助遠端功能測試與最終確認，謝謝。
```

### E.2 電腦狀態操作細節（需要時再做）
1. 主機保持開機，關閉自動睡眠。
2. 螢幕不鎖定，停在桌面或指定待命畫面。
3. 關閉與測試無關視窗（影音/更新提示/彈窗）。
4. 保留必要遠端工具，不任意重開機。
5. 若遇到阻塞，先在交接訊息補充，再通知台北窗口。
