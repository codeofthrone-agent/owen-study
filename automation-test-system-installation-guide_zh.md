...
- PC 登入密碼：`thortron`
- RobotArm 登入帳號：`er`
- RobotArm 登入密碼：`thortron`
- 適用範圍：現場安裝/維護階段，用於登入 PC、機械手臂控制端與網路連線確認
- 使用原則：**請勿修改帳號密碼**，避免台北端無法遠端檢修；若有必要變更，請先由負責人核准並同步更新文件。
...
> **本章做完你應該看到（完成樣貌）**：手機可連 `qa_rpt` 上網、PC 可連 `qa_rpt` 上網、RobotArm 控制端可連 `qa_rpt` 上網、IPCAM 至少 1 台可正常查看串流。
...
1. **[必做]** 勾選本章所有驗證項目。
2. **[必做]** 補上必要照片（全景、配線、主機畫面）。
3. **[必做]** 若有未完成項目，標記 `Blocked/Rework` 並填原因。
---
...
6. **[必做]** 請安裝人員使用手機連上 `qa_rpt`，實測可正常上網（例如可開啟任一外部網站）。
...
### 3.2.1 PC 開機後網路確認（qa_rpt）
1. **[必做]** PC 開機後，接上螢幕、鍵盤與滑鼠。
2. **[必做]** 以施工帳號登入 PC（見 0.6 帳號與密碼資訊）。
3. **[必做]** 確認目前 Wi‑Fi 連線為 `qa_rpt`。
4. **[必做]** 於 PC 端開啟瀏覽器確認可正常上網（任一外部網站可開啟）。

### 3.3 驗證標準
- [ ] 主機位置不阻礙操作與維修
- [ ] 主機電源與網路連接穩定
- [ ] 關聯線材長度合理且固定完成
- [ ] PC 已登入並確認連上 `qa_rpt`，可正常上網
...
### 5.2.1 機械手臂控制端開機與網路確認（qa_rpt）
1. **[必做]** 將螢幕、鍵盤、滑鼠接到機械手臂控制端（RobotArm 電腦）。
2. **[必做]** 開機後登入桌面（帳號密碼見 0.6）。
3. **[必做]** 確認目前 Wi‑Fi 連線為 `qa_rpt`。
4. **[必做]** 開啟瀏覽器確認可正常上網（任一外部網站可開啟）。
5. **[有問題才做]** 若未連上 `qa_rpt`，先完成連線再進行後續安裝/測試步驟。

### 5.3 驗證標準
- [ ] 機械手臂固定牢靠、活動範圍正常
- [ ] 面板安裝位置正確且可維護
- [ ] 配線完成且符合安全規範
- [ ] 機械手臂控制端（若使用 PC）已登入並確認連上 `qa_rpt`
...
|---|---|---|---|---|
| 看不到 `qa_rpt` | Wi‑Fi 未上電/位置太遠 | 先確認 Router/AP 電源與指示燈 | 改用 Extender 或調整位置後重測 | 現場安裝負責人 |
| IPCAM 無串流 | 網路未連上或供電不穩 | 先重插電源並確認連線到 `qa_rpt` | 記錄 CAM 編號，標記 `Blocked` 並交由後續測試章節處理 | 現場安裝負責人 |
| PC 偵測不到燒錄器 | USB 路徑或接頭鬆脫 | 依 `燒錄器 -> Hub -> 延長線 -> PC` 重插一次 | 更換延長線或 Hub 埠位 | 現場安裝負責人 |
| 機械手臂動作異常 | 線材干涉/接法錯誤 | 立即斷電，檢查 UART 對應與活動範圍 | 標記 `Blocked`，完成修正後再上電 | 現場安裝負責人 |
