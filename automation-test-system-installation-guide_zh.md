...
- 通用登入密碼：`thortron`
- 適用範圍：現場安裝/維護階段，用於登入 PC 或相關設備進行網路修正
- 使用原則：登入後若有變更密碼，請同步更新本文件與交接紀錄
...
### 2.2.1 IPCAM 管理 App 成員加入（SwitchBot）
> 目的：讓現場安裝者也能在管理 App 看到 IPCAM，協助定位、角度調整與後續維護。

1. 於管理者手機進入 SwitchBot「家庭成員邀請」頁面（選擇邀請方式）。
2. 優先使用 **訊息邀請** 或 **邀請碼邀請**（適合多人、可留文字紀錄）。
3. 若使用 **QR 圖碼邀請**，請注意：
   - 單一 QR 圖碼僅可邀請 1 位成員。
