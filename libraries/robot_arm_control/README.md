...
**MyCobot 280 機器手臂控制庫 - Socket 連接方式 (BDD 風格 v5.5.5)**
...
- ✅ **本機化視覺檢測** (v4.0.0):
  ...
  - 多級亮度檢測（0-100%，11 級）
  - 雙影像源（RTSP / Socket）
- ✅ **維護分析 (Maintenance Analysis)** ⭐ NEW (v5.5.5):
  - 關節移動追蹤（Joint Movement Tracking）
  - 累積移動度數統計，用於估算磨損
...
**功能**:
追蹤機器手臂 6 個關節的累積移動角度，用於估算機械磨損和維護週期分析。

**特性**:
- **推算模式**: 基於指令目標位置計算位移，完全不增加網路通訊延遲。
- **自動累計**: `send_angles` 和 `press_button` (Atomic) 自動計入統計。
- **測試報告整合**: 透過關鍵字輕鬆將統計數據寫入 Robot Framework 報告。
...
├── RobotArmKeywords.py              # Robot Framework 關鍵字庫 (v5.5.5)
