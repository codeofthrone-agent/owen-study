...
本目錄包含以下三個核心組件，協同完成多模態檢測任務：

1.  **`VoiceAssistantDetection.py` (核心協調者)**
    - **角色**：測試流程的指揮官。
    - **功能**：整合 `voice_control`、`ipcam_light_detection` 與 `SerialLogParser`。透過多執行緒技術，同時發起語音指令並監控視覺（螢幕亮度）與聽覺（日誌）回饋，最後根據邏輯（如 AND/OR）判斷測試結果。
    - **Robot 關鍵字**：提供高階業務關鍵字，如 `測試語音助理回應`。

2.  **`SerialLogParser.py` (日誌解析器)**
    - **角色**：背景情報員。
    - **功能**：持續監聽 UART 串列埠日誌。它能即時從海量的系統日誌中，精準擷取與播放狀態或特定事件相關的模式（Patterns），為測試提供底層證據。

3.  **`RemoteSystemConfigValidator.py` (系統校準器)**
    - **角色**：環境診斷技師。
    - **功能**：在測試開始前檢查遠端設備的系統配置（如 `/etc/init.d/emmc`）。若發現設定錯誤（如日誌未導向正確位置），能自動進行備份、修正並提示重開機，確保測試環境的標準化與一致性。

---

## 遠端配置驗證流程

`RemoteSystemConfigValidator` 的運作邏輯如下圖所示（包含 `auto_kill_conflicts` 機制）：
...
+-----------------------------+
      ...
      v
+-----------------------------+      YES      +---------------------------------------+
| 是否有設定                   |------------->| _terminate_conflicting_serial_sessions()|
| auto_kill_conflicts=True?   |               | (掃描並中止占用該 Port 的 screen/tio)    |
+-----------------------------+               +---------------------------------------+
      | NO                                                      |
      +-------------------------+-------------------------------+
                                |
...
+-----------------------------+
              ...
              v
+-----------------------------+
| 再次 check_file_config()     |
+-----------------------------+
              |
        +-----+-----[ 修正成功 ]------>[ 結束: needs_reboot=True ]
        |
        +-----------[ 修正失敗 ]------>[ 結束: error_message ]
```
