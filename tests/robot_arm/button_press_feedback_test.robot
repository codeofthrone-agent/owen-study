...
Suite Setup       Setup Robot Connection
Suite Teardown    Teardown Robot Connection
...
按壓反饋測試: Light2 按鈕
    ...
    ${before_panel_result}=    When 用戶檢測面板按鈕 "light2" 的顏色    save_debug_image=${True}    step_prefix=step2_before
    ...
    ${after_panel_result}=    When 用戶檢測面板按鈕 "light2" 的顏色    save_debug_image=${True}    step_prefix=step5_after
...
Setup Robot Connection
    ...
    When 連接機器手臂    ${SERVER_HOST}    ${SERVER_PORT}
    Log To Console    ✓ 已連接到 ${SERVER_HOST}:${SERVER_PORT}

Teardown Robot Connection
    [Documentation]    斷開連接
    When 斷開機器手臂連接
