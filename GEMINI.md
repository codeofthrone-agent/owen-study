...
25. 提出計畫及進行思考都使用中文
26. 每一個python腳本在開頭都必須有詳細的註解，包含開發用途、日期、功能、使用方式等 
27. **優先使用 Subagents 工具**：在執行複雜的文件處理、腳本編寫或系統操作時，應優先考慮使用 `shell`, `fs`, `scripting` 等專門的 subagents 工具。這些工具能更精確地處理大型文件與複雜邏輯，減少手動錯誤並提升自動化效率。
